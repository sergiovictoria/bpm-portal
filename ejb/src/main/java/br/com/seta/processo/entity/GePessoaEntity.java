...
@NamedNativeQueries({  
        @NamedNativeQuery(name = "find.cnpjCpf",        query = "SELECT * FROM GE_PESSOA GEP WHERE GEP.NROCGCCPF = :cnpjCpf AND GEP.DIGCGCCPF = :digCnpjCpf", resultClass = GePessoaEntity.class),
        @NamedNativeQuery(name = "find.SQL_BUSCA_GE_PESSOA_POR_SEQPESSOA_OU_CPF_CNPJ", query =  "SELECT g.* FROM GE_PESSOA g, MAF_FORNECEDOR f WHERE f.seqfornecedor = g.seqpessoa AND f.statusgeral = 'A' AND f.seqfornecedor || concat(g.nrocgccpf, g.digcgccpf) LIKE :filtro ORDER BY g.nomerazao", resultClass = GePessoaEntity.class),
        @NamedNativeQuery(name = "find.SQL_CNPJ",       query =  "SELECT GEP.* FROM GE_PESSOA GEP, MAF_FORNECEDOR FON WHERE GEP.SEQPESSOA = FON.SEQFORNECEDOR AND FON.STATUSGERAL = 'A' AND GEP.NROCGCCPF LIKE :filtro ORDER BY GEP.NOMERAZAO", resultClass = GePessoaEntity.class),
        @NamedNativeQuery(name = "find.SQL_RAZAO",      query =  "SELECT GEP.* FROM GE_PESSOA GEP, MAF_FORNECEDOR FON WHERE GEP.SEQPESSOA = FON.SEQFORNECEDOR AND FON.STATUSGERAL = 'A' AND GEP.NOMERAZAO LIKE :filtro ORDER BY GEP.NOMERAZAO", resultClass = GePessoaEntity.class),
        @NamedNativeQuery(name = "find.SQL_ALL",        query =  "SELECT GEP.* FROM GE_PESSOA GEP, MAF_FORNECEDOR FON WHERE GEP.SEQPESSOA = FON.SEQFORNECEDOR AND FON.STATUSGERAL = 'A' ORDER BY GEP.NOMERAZAO", resultClass = GePessoaEntity.class),
        @NamedNativeQuery(name = "find.ID",             query =  "SELECT GEP.* FROM GE_PESSOA GEP, MAF_FORNECEDOR FON WHERE GEP.SEQPESSOA = FON.SEQFORNECEDOR AND FON.STATUSGERAL = 'A' AND GEP.SEQPESSOA = :filtro", resultClass = GePessoaEntity.class)
})


@NamedQueries({
	@NamedQuery(name = "find.SQL_BUSCA_GE_PESSOA_ALL", query = "SELECT g FROM GePessoaEntity AS g,  MafFornecedorEntity AS f WHERE f.seqfornecedor = g.seqpessoa AND f.statusgeral = 'A' ORDER BY g.nomerazao"),
	@NamedQuery(name = "find.SQL_BUSCA_GE_PESSOA_POR_FORNECEDOR", query = "SELECT g FROM GePessoaEntity AS g, MafFornecedorEntity AS f WHERE f.seqfornecedor = g.seqpessoa AND f.statusgeral = 'A' AND f.seqfornecedor = :filtro"),
	@NamedQuery(name = "find.SQL_BUSCA_GE_PESSOA_POR_SEQPESSOA",  query =  "SELECT g FROM GePessoaEntity AS g, MafFornecedorEntity AS f WHERE f.seqfornecedor = g.seqpessoa AND f.statusgeral = 'A' AND f.seqfornecedor LIKE :filtro ORDER BY g.nomerazao"),
	@NamedQuery(name = "find.SQL_BUSCA_GE_PESSOA_POR_NOMERAZAO",  query =  "SELECT g FROM GePessoaEntity AS g, MafFornecedorEntity AS f WHERE f.seqfornecedor = g.seqpessoa AND f.statusgeral = 'A' AND g.nomerazao LIKE :filtro ORDER BY g.nomerazao")
})

@Cacheable(true)
public class GePessoaEntity implements java.io.Serializable {
	...
	public GePessoaEntity() {
	}

	public GePessoaEntity(BigDecimal seqpessoa, String status, String nomerazao, long nrobaseexportacao) {
	...
	public GePessoaEntity(BigDecimal seqpessoa, Byte versao, String status, Date dtaativacao, String nomerazao, String fantasia, String palavrachave,
			...
			String indgeraressarcstsn, MafFornecedorEntity mafFornecedorEntity) {
		this.seqpessoa = seqpessoa;
		this.versao = versao;
		this.status = status;
	...
	@Column(name = "INDTIPOPESSOACALCPISCOFINS", length = 1)
	public String getIndtipopessoacalcpiscofins() {
		return this.indtipopessoacalcpiscofins;
	}
	...
	@Column(name = "INDTIPOPESSOACALIPI", length = 1)
	public String getIndtipopessoacalipi() {
		return this.indtipopessoacalipi;
	}
	...
	@Column(name = "INDGERARESSARCSTSN", length = 1)
	public String getIndgeraressarcstsn() {
		return this.indgeraressarcstsn;
	}
